...
function getNextId(callback) {
    var params = {
        TableName: 'Preguntas', // Reemplaza 'Preguntas' con el nombre de tu tabla en DynamoDB
        ProjectionExpression: 'ID', // Solo necesitamos recuperar el campo ID
        Limit: 1
    };
    ...
    dynamodb.scan(params, function(err, data) {
        if (err) {
            ...
            var nextId = 1; // El primer ID disponible será 1 si no hay registros en la tabla

            if (data.Items.length > 0) {
                var lastId = parseInt(data.Items[0].ID.N);
                nextId = lastId + 1; // Incrementar el último ID utilizado para obtener el siguiente
            }
